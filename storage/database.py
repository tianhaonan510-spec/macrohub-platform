...
def init_db() -> None:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"未找到标准化数据：{csv_path}")
    df = pd.read_csv(csv_path)
    DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(DB_PATH) as conn:
        df.to_sql("macro_observations", conn, if_exists="replace", index=False)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_macro_query ON macro_observations(country_code, indicator_code, frequency, date)")
